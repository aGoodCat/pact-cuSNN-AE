...
#include "../inc/scnn.h"
__device__ void conv_1_64_56_56_64_switch_function( unsigned int switch_condition,float *temp_kernel,float v,float *temp_result){
...
__global__ void conv_1_64_56_56_64_transform(float *matrix, float *d_nnz, char *ids, unsigned int *c_lens){
    unsigned int global_id = blockIdx.x * 512 + threadIdx.x;
    unsigned int batch_id = global_id/(64*56*56);
    if(global_id >= 1*56*56*64){
        return ;
    }
    ...
    int h = ((global_id - batch_id * 64 * 56 * 56)/64)/56+1;
    int w = ((global_id - batch_id * 64 * 56 * 56)/64)%56+1;
    ...
    for(int tile_h_id = th_start;tile_h_id>=0;tile_h_id--){
        ...
        for(int tile_w_id = tw_start;tile_w_id>=0;tile_w_id--){
            ...
            unsigned int index = atomicAdd(&c_lens[batch_id*64*224+c*224+tile_id],1);
            unsigned int abs_h = h - tile_h_id*2;
            unsigned int abs_w = w - tile_w_id*7;
            d_nnz[batch_id*64*224*(2+3-1)*(7+3-1)+
                  c*224*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+index] = v;
            ids[batch_id*64*224*(2+3-1)*(7+3-1)+
                c*224*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+index] = abs_h*(7+3-1)+abs_w;
        }
    }
}
__device__ void conv_1_64_56_56_64_load_data_2_register(float *__restrict__ data_array, unsigned int c_index, const float * __restrict__ kernel, unsigned int n_id){
...
__device__ void conv_1_64_56_56_64_load_input_2_shared_memory(float *values,unsigned int *c_lens,char *ids,
    ...
    for(unsigned int c_id=warp_id;c_id<8&&tile_c_id+c_id<64;c_id+=2){
        ...
        for(unsigned int id = lane_id;id<end_index;id+=32){
            shared_input[c_id*(2+3-1)*(7+3-1)+id] = values[batch_id*64*224*(2+3-1)*(7+3-1)+
                                                           (tile_c_id+c_id)*224*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+id];
            shared_ids[c_id*(2+3-1)*(7+3-1)+id] = ids[batch_id*64*224*(2+3-1)*(7+3-1)+
                                                      (tile_c_id+c_id)*224*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+id];
        }
    }
}
__global__ void conv_1_64_56_56_64_conv2d(float * __restrict__ values, unsigned int * __restrict__ c_lens,
    ...
    conv_1_64_56_56_64_load_input_2_shared_memory(values,c_lens,ids,input,input_ids,channel_lens,warp_id,lane_id,batch_id,t_id,start_channel_index);
    __syncthreads();
    float v;
    unsigned int id;
    for(unsigned int n = threadIdx.x;n<64;n+=64){
        for(unsigned int c=start_channel_index;c<start_channel_index+8&&c<64;c++){
            ...
            conv_1_64_56_56_64_load_data_2_register(data_array,(c),kernel,n);
            ...
            for(unsigned int iter=0;iter<iters;iter++) {
                ...
                conv_1_64_56_56_64_switch_function(id,data_array,v,temp_result);
            }
        }
        for (unsigned int th = 0; th < 2; ++th) {
            for (unsigned int tw = 0; tw < 7; ++tw) {
                ...
                atomicAdd(&outputs[batch_id * 64 * 56 * 56 + (tile_h_id + th) * 56 * 64 + (tile_w_id + tw) * 64 +
