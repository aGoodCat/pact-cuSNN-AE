...
void Conv_1_128_112_112_128::initialize(unsigned int b,unsigned int c,unsigned int h,unsigned int w,unsigned int n,
...
__device__ void conv_1_128_112_112_128_switch_function( unsigned int switch_condition,float *temp_kernel,float v,float *temp_result){
...
__global__ void conv_1_128_112_112_128_transform(float *matrix, float *d_nnz, char *ids, unsigned int *c_lens){
    ...
    unsigned int batch_id = global_id/(128*112*112);
    if(global_id >= 1*112*112*128){
        return ;
    }
    ...
    int h = ((global_id - batch_id * 128 * 112 * 112)/128)/112+1;
    int w = ((global_id - batch_id * 128 * 112 * 112)/128)%112+1;
    int th_start = min(h/2,56-1);
    int tw_start = min(w/7,16-1);
    for(int tile_h_id = th_start;tile_h_id>=0;tile_h_id--){
        ...
        for(int tile_w_id = tw_start;tile_w_id>=0;tile_w_id--){
            ...
            unsigned int tile_id = tile_h_id * 16 + tile_w_id;
            unsigned int index = atomicAdd(&c_lens[batch_id*128*896+c*896+tile_id],1);
            unsigned int abs_h = h - tile_h_id*2;
            unsigned int abs_w = w - tile_w_id*7;
            d_nnz[batch_id*128*896*(2+3-1)*(7+3-1)+
                  c*896*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+index] = v;
            ids[batch_id*128*896*(2+3-1)*(7+3-1)+
                c*896*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+index] = abs_h*(7+3-1)+abs_w;
        }
    }
}
...
__device__ void conv_1_128_112_112_128_load_input_2_shared_memory(float *values,unsigned int *c_lens,char *ids,
    ...
    for(unsigned int c_id=warp_id;c_id<8&&tile_c_id+c_id<128;c_id+=4){
        unsigned int end_index = c_lens[batch_id*128*896+(tile_c_id+c_id)*896+tile_id];
        if(lane_id ==0){
            shared_lens[c_id] = end_index;
        }
        for(unsigned int id = lane_id;id<end_index;id+=32){
            shared_input[c_id*(2+3-1)*(7+3-1)+id] = values[batch_id*128*896*(2+3-1)*(7+3-1)+
                                                           (tile_c_id+c_id)*896*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+id];
            shared_ids[c_id*(2+3-1)*(7+3-1)+id] = ids[batch_id*128*896*(2+3-1)*(7+3-1)+
                                                      (tile_c_id+c_id)*896*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+id];
        }
    }
}
__global__ void conv_1_128_112_112_128_conv2d(float * __restrict__ values, unsigned int * __restrict__ c_lens,
    ...
    const unsigned int batch_id = (blockIdx.x/(16*896));
    const unsigned int t_id = (blockIdx.x - batch_id*16*896)/16;
    const unsigned int tile_h_id = (t_id / 16)*2;
    const unsigned int tile_w_id = (t_id % 16)*7;
    ...
    conv_1_128_112_112_128_load_input_2_shared_memory(values,c_lens,ids,input,input_ids,channel_lens,warp_id,lane_id,batch_id,t_id,start_channel_index);
    __syncthreads();
    float v;
    unsigned int id;
    for(unsigned int n = threadIdx.x;n<128;n+=128){
        for(unsigned int c=start_channel_index;c<start_channel_index+8&&c<128;c++){
            ...
            conv_1_128_112_112_128_load_data_2_register(data_array,(c),kernel,n);
            ...
            for(unsigned int iter=0;iter<iters;iter++) {
                ...
                conv_1_128_112_112_128_switch_function(id,data_array,v,temp_result);
            }
        }
        for (unsigned int th = 0; th < 2; ++th) {
            for (unsigned int tw = 0; tw < 7; ++tw) {
                if (tile_h_id + th >= 112 || tile_w_id + tw >= 112) {
                    continue;
                }
                atomicAdd(&outputs[batch_id * 128 * 112 * 112 + (tile_h_id + th) * 112 * 128 + (tile_w_id + tw) * 128 +
...
float * Conv_1_128_112_112_128::forward(float *input) {
    cudaMemset(output, 0, B*N*hOut*wOut*sizeof(float));
    cudaMemset(c_lens, 0, B*C*TILES_EACH_CHANNEL*sizeof(unsigned int));
    conv_1_128_112_112_128_transform<<<(B*C*H*W-1)/512+1,512>>>(input,this->nnz,this->ids,this->c_lens);
    conv_1_128_112_112_128_conv2d<<<B*TCS*TILES_EACH_CHANNEL,N>>>(this->nnz,this->c_lens,this->ids,this->kernel,this->output);
